                     ...
                     - the signature of the io.RenderDrawListsFn handler has changed! 
                       ...
                       became:
                            ImGui_XXXX_RenderDrawLists(ImDrawData* draw_data). 
                              ...
                              ImDrawCmd  'texture_id'       -> 'RendererID'
...
void ImGui::NewFrame()
{
    ...
    g.OverlayDrawList.PushRendererID(g.IO.Fonts->TexID);
...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.IO.MouseDrawCursor)
        {
            ...
            const ImRendererID tex_id = g.IO.Fonts->TexID;
            g.OverlayDrawList.PushRendererID(tex_id);
            ...
            g.OverlayDrawList.PopRendererID();
        }
...
bool ImGui::Begin(const char* name, bool* p_opened, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->DrawList->PushRendererID(g.Font->ContainerAtlas->TexID);
...
void ImGui::PushFont(ImFont* font)
{
    ...
    g.CurrentWindow->DrawList->PushRendererID(font->ContainerAtlas->TexID);
}

void  ImGui::PopFont()
{
    ImGuiState& g = *GImGui;
    g.CurrentWindow->DrawList->PopRendererID();
...
void ImGui::Image(ImRendererID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ImGui::ImageButton(ImRendererID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
        ...
        struct Funcs
        {
            static void NodeDrawList(ImDrawList* draw_list, const char* label)
            {
                ...
                for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
                {
                    ...
                    {
                        ImGui::BulletText("Draw %d indexed vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)", pcmd->ElemCount, pcmd->RendererID, pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
