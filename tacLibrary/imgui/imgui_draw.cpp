...
void ImDrawList::AddDrawCmd()
{
    ...
    draw_cmd.RendererID = _RendererIDStack.Size ? _RendererIDStack.back() : NULL;
    draw_cmd.UserCallback = NULL;
    draw_cmd.UserCallbackData = NULL;

    //if( draw_cmd.ClipRect.x <= draw_cmd.ClipRect.z && draw_cmd.ClipRect.y <= draw_cmd.ClipRect.w )
    //{
...
void ImDrawList::UpdateRendererID()
{
    ImDrawCmd* current_cmd = CmdBuffer.Size ? &CmdBuffer.back() : NULL;
    const ImRendererID texture_id = _RendererIDStack.Size ? _RendererIDStack.back() : NULL;
    if (!current_cmd || (current_cmd->ElemCount != 0 && current_cmd->RendererID != texture_id) || current_cmd->UserCallback != NULL)
        AddDrawCmd();
    else
        current_cmd->RendererID = texture_id;
}

void ImDrawList::PushRendererID(const ImRendererID& texture_id)
{
    _RendererIDStack.push_back(texture_id);
    UpdateRendererID();
}

void ImDrawList::PopRendererID()
{
    IM_ASSERT(_RendererIDStack.Size > 0);
    _RendererIDStack.pop_back();
    UpdateRendererID();
}
...
void ImDrawList::AddText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
    ...
    IM_ASSERT(font->ContainerAtlas->TexID == _RendererIDStack.back());  // Use high-level ImGui::PushFont() or low-level ImDrawList::PushRendererID() to change font.
...
void ImDrawList::AddImage(ImRendererID user_texture_id, const ImVec2& a, const ImVec2& b, const ImVec2& uv0, const ImVec2& uv1, ImU32 col)
{
    if ((col >> 24) == 0)
        return;

    // FIXME-OPT: This is wasting draw calls.
    const bool push_texture_id = _RendererIDStack.empty() || user_texture_id != _RendererIDStack.back();
    if (push_texture_id)
        PushRendererID(user_texture_id);
